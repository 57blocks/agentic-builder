...
function buildFrontendPrompt(ctx: PromptContext): string {
  const sections: string[] = [
    ...
    `- **Typed API surface (HARD RULE): call the backend ONLY through the generated typed client** — \`import { api } from "@/api/endpoints"\` then \`api.<service>.<method>(...)\` (e.g. \`api.enrollments.createPrivate(body)\`, \`api.courses.getByCourseId(courseId)\`). It is GENERATED from the shared schema's ENDPOINTS registry and is READ-ONLY: never edit \`api/endpoints.ts\`, never hand-write \`fetch\`/axios, never create a parallel HTTP wrapper, and never import the low-level \`apiClient\` directly in a view. A call to a missing endpoint or a wrong request/response field is then a COMPILE error (tsc gate) instead of a dead button at runtime.`,
    `- Find the method for an endpoint by its doc: every \`api.<service>.<method>\` carries a \`/** METHOD /path */\` comment mirroring the contract. Read \`frontend/src/api/endpoints.ts\` to pick it — request/response types and path params are already baked in, so never assemble raw URL strings anywhere.`,
    ...
    `- Use \`useEffect\` + loading/error state for all data fetching. Read \`frontend/src/api/endpoints.ts\` first to pick the right \`api.<service>.<method>\`.`,
