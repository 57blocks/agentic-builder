...
function buildFrontendPrompt(ctx: PromptContext): string {
  const sections: string[] = [
    ...
    `- **API paths (ONE rule): the client base URL already carries the FULL API prefix (e.g. \`/api/v1\`). Pass ONLY the business path** — never include \`/api\` OR a version segment. Derive the call path from an API_CONTRACTS.json endpoint by stripping the base prefix: contract \`/api/v1/users/me\` → \`apiClient.get("/users/me")\`. Read the Project Convention Card for the exact base.`,
