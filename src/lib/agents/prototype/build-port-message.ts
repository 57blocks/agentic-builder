...
export function buildPortMessage(input: PortMessageInput): string {
  ...
  return [
    ...
    `- Reproduce EVERY element from the captured markup, INCLUDING all \`<img>\` tags with`,
    `  their exact \`src\` / \`srcSet\` / \`alt\` (in JSX use \`srcSet\`). Do NOT drop, skip,`,
    `  summarize, or collapse any content, images, or sections.`,
    `- CONTENT FIDELITY: use ONLY the text, data, labels and values that appear in the`,
    `  captured markup. Do NOT invent content the demo doesn't show — no made-up prices,`,
    `  dates, names, counts, statuses, or extra info fields. If the demo doesn't show a`,
    `  value, don't add one.`,
