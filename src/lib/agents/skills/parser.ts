...
function parseTrigger(raw: unknown, filePath: string): SkillTrigger {
  ...
  if (type === "context") {
    ...
    if (anyFeatures.length === 0 && anyEnv.length === 0 && allFlags.length === 0) {
      throw new Error(
        `${filePath}: context trigger requires at least one of "any_of_features", "any_of_env_keys", "all_of_flags".`,
      );
    }
    return {
      type: "context",
