/**
 * Canonical auth hook for the magic-link scaffold.
 *
 ...
 *   - `requestLink / consumeToken / logout / refresh / clearError`
 *                                          — bound store actions
 *
...
export interface UseAuthResult {
  ...
  requestLink: ReturnType<typeof useAuthStore.getState>["requestLink"];
  consumeToken: ReturnType<typeof useAuthStore.getState>["consumeToken"];
...
export function useAuth(): UseAuthResult {
  ...
  const consumeToken = useAuthStore((s) => s.consumeToken);
