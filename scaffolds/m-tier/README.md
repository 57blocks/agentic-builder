...
- **缺组件就自造**：脚手架预装了常用 shadcn 组件（Button / Input / Label / Textarea / Card / Badge / Dialog / Select / Dropdown-menu / Checkbox / Tabs / Tooltip / Table / Form / Sonner）。需要更多组件时，用 `npx shadcn@latest add <name>` 添加，或在 `components/ui/` 手写（可用 Radix 原语）；同样优先 `cn()` + 语义 token，尽量避免行内 `style={{}}`。
